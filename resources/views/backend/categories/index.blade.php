                ...
                <div class="card card-custom">
                    <div class="card-header flex-wrap border-0 pt-6 pb-0">
                        <div class="card-title">
                            <h3 class="card-label">{{ isset($top_cat->name) ? $top_cat->name : 'Categories' }}
                                    ...
                                    @if (count($categories))
                                        @foreach ($categories as $key => $category)
                                            <tr data-id="{{ $category->id }}" class="sort-tr">
                                                <td>{{ ++$key }}</td>
                                                <td>
                                                    <input type="checkbox" name="nav_check" class="nav_check" @if (isset($category->is_navbar) && $category->is_navbar == '1') data-status="0" @else data-status="1" @endif @if (isset($category->is_navbar) && $category->is_navbar == '1')
                                                ...
                                                <td>
                                                    ...
                                                    @elseif (request()->type == 'Subcategory')
                                                        @php
                                                            ...
                                                            $id = $category->id;
                                                        @endphp
                                                    @elseif (request()->type == 'Subcategory1')
                                                        @php
                                                            ...
                                                            $id = $category->id;
                                                        @endphp
                                                    @elseif (request()->type == 'Subcategory2')
                                                        @php
                                                            $type = 'Topics';
                                                            $id = $category->id;
                                                        @endphp
                                                    @elseif (request()->type == 'Topics')
                                                        @php
                                                            $type = 'Questions';
                                                            $id = $category->id;
                                                        @endphp
                                                    ...
                                                    @if ($type == 'Questions')
                                                        ...
                                                        @endphp
                                                        <a href="{{ route('admin.questioncat', $category->id) }}{{ isset($question->question_id) ? "?question_id=$question->question_id" : '' }}"><i
                                                                class="fa fa-eye" aria-hidden="true"></i>
                                                        </a>
                                                    @else
                                                        <a href="{{ route('category_data', ['type' => $type, 'id' => $id]) }}"
                                                            style="text-decoration: none; color:green;">
